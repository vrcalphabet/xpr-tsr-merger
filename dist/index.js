...
var FileManager = class {
  ...
  static dirPath(filePath) {
    return import_path.default.extname(filePath) ? import_path.default.dirname(filePath) : filePath;
  }
  /**
   * パスを結合します。
   * @param path 結合するパス
   ...
   * @returns ディレクトリの名前
   */
  static directories(filePath) {
    const dirPath = this.dirPath(filePath);
    return import_fs.default.readdirSync(dirPath).filter((directory) => import_fs.default.statSync(this.join(dirPath, directory)).isDirectory());
  }
  /**
   * ファイルが存在するかを確認します。
  ...
  static makeDir(dirPath) {
    const dir = import_path.default.dirname(dirPath);
   ...
   */
  static async readFileAsStream(filePath, onDataHandler, onEndHandler) {
    return new Promise((resolve) => {
      const stream = import_fs.default.createReadStream(filePath);
      stream.on("data", (stream2) => onDataHandler(stream2));
...
// src/utils/DirectoryFilter.ts
var DirectoryFilter = class {
  /**
...
var HashGenerator = class {
   ...
   */
  static async generateFromFile(...paths) {
...
// src/common/XprErrorMessages.ts
var XprErrorMessages_default = {
  GENERAL: {
...
// src/common/XprRegExp.ts
var XprRegExp_default = {
  COMMENT: /%-.*?-%|%.*?(?=\n)/gs,
  IDENTIFIER: /^[A-Z0-9_]+$/,
  DIRECTORY_PATH: /^\/.*$/,
  KEY: /^[A-Z0-9_]+$/,
...
// src/xpr/XprMetadataBuilder.ts
var XprMetadataBuilder = class _XprMetadataBuilder {
  /** トークンの配列 */
  tokens;
  /** 現在のトークン */
  token;
  /** シングルトンのインスタンス */
  static INSTANCE = new _XprMetadataBuilder();
  constructor() {
    this.token = null;
  }
  /** インスタンスを取得します。 */
  ...
  buildTree(tokens) {
    ...
    while (true) {
      this.nextToken();
      if (this.token === null) {
        this.error(XprErrorMessages_default.GENERAL.MISSING_METADATA);
        return null;
      }
      switch (this.token) {
        // メタデータ
        case "@name":
          name = this.parseToken(name, this.parseName, XprErrorMessages_default.NAME);
          if (name === null) return null;
          break;
        case "@includes":
          includes = this.parseToken(includes, this.parseIncludes, XprErrorMessages_default.INCLUDES);
          if (includes === null) return null;
          break;
        case "@excludes":
          excludes = this.parseToken(excludes, this.parseExcludes, XprErrorMessages_default.INCLUDES);
  ...
  parseName() {
    ...
    if (this.token === null || this.validateToken(",")) {
      this.error(XprErrorMessages_default.NAME.MISSING_IDENTIFIER);
      return null;
    }
    if (!this.validateRegex(XprRegExp_default.IDENTIFIER)) {
      this.error(XprErrorMessages_default.NAME.INVALID_FORMAT);
      return null;
    }
    const name = this.token;
    this.nextToken();
    if (!this.validateToken(",")) {
      this.error(XprErrorMessages_default.NAME.MISSING_COMMA);
      return null;
    }
    return name;
  }
   ...
   */
  parseIncludes() {
    const directories = this.parseDirectories(true, XprErrorMessages_default.INCLUDES);
   ...
   */
  parseExcludes() {
    const directories = this.parseDirectories(false, XprErrorMessages_default.EXCLUDES);
    if (directories === null) return null;
    return directories;
  }
  /**
  ...
  parseDirectory(ERROR_BLOCK) {
    ...
    if (!this.validateRegex(XprRegExp_default.DIRECTORY_PATH)) {
...
// src/xpr/XprNodeBuilder.ts
var XprNodeBuilder = class _XprNodeBuilder {
  /** トークンの配列 */
  tokens;
  /** 現在のトークン */
  token;
  /** シングルトンのインスタンス */
  static INSTANCE = new _XprNodeBuilder();
  constructor() {
    this.token = null;
  }
  /** インスタンスを取得します。 */
  ...
  recursive(baseMulti) {
    ...
    while (true) {
      this.nextToken();
      if (this.token === null) {
        this.error(XprErrorMessages_default.GENERAL.INVALID_TOKEN_END);
        return null;
      }
      ...
      switch (type) {
        ...
        case 4 /* BRACKET_OPEN */:
          ...
          break;
        case 5 /* BRACKET_CLOSE */:
          if (key === null || xpath === null) {
            this.error(XprErrorMessages_default.NODE.MISSING_KEY_OR_XPATH);
            return null;
          }
          if (nodes.length === 0) {
            this.error(XprErrorMessages_default.NODE.MISSING_NODE);
            return null;
          }
          return {
            key,
        ...
        case 6 /* COMMA */:
          if (xpath === null) {
            this.error(XprErrorMessages_default.NODE.MISSING_XPATH);
            return null;
          }
          return {
            key,
   ...
   */
  checkType() {
    if (this.validateRegex(XprRegExp_default.KEY)) {
      return 0 /* KEY */;
    }
    if (this.validateRegex(XprRegExp_default.XPATH)) {
      return 1 /* XPATH */;
    }
    if (this.validateRegex(XprRegExp_default.MULTI)) {
      return 2 /* MULTI */;
    }
    if (this.validateRegex(XprRegExp_default.ATTRIBUTE)) {
      return 3 /* ATTRIBUTE */;
    }
    if (this.validateToken("{")) {
      return 4 /* BRACKET_OPEN */;
    }
    if (this.validateToken("}")) {
      return 5 /* BRACKET_CLOSE */;
    }
    if (this.validateToken(",")) {
      return 6 /* COMMA */;
    }
    this.error(XprErrorMessages_default.GENERAL.INVALID_TOKEN);
    return null;
  }
  /**
   * 現在のトークンが指定した文字であるかどうかを判定します。
...
// src/xpr/XprBuilder.ts
var XprBuilder = class {
  /**
...
// src/xpr/XprTokenizer.ts
var XprTokenizer = class {
  /**
   * 入力された文字列をトークンに分割します。
  ...
  static tokenize(input) {
    const strippedInput = input.replaceAll(XprRegExp_default.COMMENT, "");
...
// src/xpr/XprConverter.ts
var XprConverter = class {
  /**
...
var FileMerger = class {
   ...
   */
  static readFileInFolders(dirPath, directories, fileName, converter) {
    const result = [];
...
// src/utils/RuleMerger.ts
var RuleMerger = class extends FileMerger {
  static merge(dirPath, directories, pretty) {
    const data = FileMerger.readFileInFolders(
      dirPath,
      directories,
      "rule.xpr",
      XprConverter
  ...
  static stringify(data, pretty) {
    return JsonFormatter.stringify(data, pretty);
  }
};

// src/trans/TransConverter.ts
var TransConverter = class {
  /**
   ...
   */
  static convert(input, directory) {
    try {
      return [directory.toUpperCase(), JSON.parse(input)];
    } catch (e) {
...
// src/utils/TransMerger.ts
var TransMerger = class extends FileMerger {
  static merge(dirPath, directories, pretty) {
    const data = FileMerger.readFileInFolders(
      dirPath,
      directories,
...
// src/index.ts
function activate() {
  vscode2.commands.registerCommand("extension.mergeXprAndTsrFiles", async () => {
    try {
