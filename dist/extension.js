...
var FileReader = class {
  ...
  static getSubdirectories(directoryPath) {
    const items = import_fs.default.readdirSync(directoryPath);
  ...
  static readFileInFolders(baseDirectoryPath, directories, fileName, callbackFn) {
    ...
    for (const directory of directories) {
      const folderPath = import_path.default.join(baseDirectoryPath, directory);
...
// src/MergeFiles.ts
var MergeFiles = class _MergeFiles {
  /** アクティブなテキストエディタ */
  editor;
  /** `index.json`のパス */
  indexFilePath = "";
  ...
  outputPath = "";
  /** シングルトンのインスタンス */
  static INSTANCE = new _MergeFiles();
  constructor() {
  }
  /** インスタンスを取得します */
  static getInstance() {
  ...
  setIndex() {
    ...
    const fileName = import_path2.default.basename(this.indexFilePath);
   ...
   */
  setPath() {
    const basePath = import_path2.default.dirname(this.indexFilePath);
    this.inputPath = import_path2.default.join(basePath, this.index.input);
    this.outputPath = import_path2.default.join(basePath, this.index.output);
   ...
   */
  getIndex(path3) {
    const index = FileReader.readFileContent(path3);
  ...
  writeFile(fileName, content) {
    const rulePath = import_path2.default.join(this.outputPath, fileName);
...
// src/extension.ts
function activate() {
  vscode3.commands.registerCommand("extension.mergeXprAndTsrFiles", () => {
    try {
